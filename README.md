...
![GitHub Stars](https://img.shields.io/github/stars/deus-h/claudeus-memory.svg?style=social)

Claudeus Memory is a knowledge graph server designed for AI models, with a focus on text-based RPGs and interactive storytelling. It helps AI maintain consistent, structured memory across conversations, enabling richer and more dynamic interactions.
...
Schemas are the heart of Claudeus Memory. They define the structure of your data and drive the automatic generation of tools.

##### Schema File Location

Place your schema files (`.schema.json`) in the `dist/data/schemas` directory of your built Claudeus Memory project. Claudeus Memory will automatically detect and process these files on startup.
...
Based on this schema, Claudeus Memory automatically creates:
* add_npc: To add new NPC nodes.
* update_npc: To modify existing NPC nodes.
* delete_npc: To remove NPC nodes.

Claudeus Memory includes 11 pre-built schemas designed for text-based RPGs, providing a ready-to-use foundation for game development.

##### SchemaManager Tool

Claudeus Memory includes a [SchemaManager tool](https://github.com/CheMiguel23/memorymesh/blob/main/SchemaManager.html) to simplify schema creation and editing. It provides a visual interface, making it easy to define your data structures without writing JSON directly.

<img width="370" alt="image" src="https://github.com/user-attachments/assets/e8f0c808-2ff6-48da-ac7c-cf51aebde7b8">

### Dynamic Tools

Claudeus Memory simplifies interaction with your knowledge graph through **dynamic tools**. These tools are not manually coded but are **automatically generated** directly from your **schema definitions**. This means that when you define the structure of your data using schemas, Claudeus Memory intelligently creates a set of tools tailored to work with that specific data structure.

**Think of it like this:** You provide a blueprint (the schema), and Claudeus Memory automatically constructs the necessary tools to build, modify, and remove elements based on that blueprint.

#### How does it work behind the scenes?

Claudeus Memory has an intelligent system that reads your schema definitions. It analyzes the structure you've defined, including the properties of your entities and their relationships. Based on this analysis, it automatically creates a set of tools for each entity type:
...
These tools are then made available through a central hub within Claudeus Memory, ensuring they can be easily accessed and used by any connected client or AI.

**In essence, Claudeus Memory's dynamic tool system provides a powerful and efficient way to manage your knowledge graph, freeing you to focus on the content and logic of your application rather than the underlying mechanics of data manipulation.**
...
The Memory Viewer is a separate tool designed to help you visualize and inspect the contents of the knowledge graph managed by Claudeus Memory. It provides a user-friendly interface for exploring nodes, edges, and their properties.
...
* Choose File: Navigate to your Claudeus Memory project directory and select the `memory.json` file (located in `dist/data/memory.json` by default).
...
1. **Clone the Repository:**
    ...
    ```bash
    git clone https://github.com/CheMiguel23/claudeus-memory.git
    cd claudeus-memory
    ```
...
5. **Configure Claude Desktop:**
    ...
    * Add an entry for `claudeus-memory` to the `mcpServers` section. You can choose **one** of the following configuration options:

    ```json
    "mcpServers": {
      "claudeus-memory": {
        "command": "node", 
        "args": ["/ABSOLUTE/PATH/TO/YOUR/PROJECT/claudeus-memory/dist/index.js"]
      }
    }
    ```

    *   Replace `/ABSOLUTE/PATH/TO/YOUR/PROJECT/` with the **actual absolute path** to your `claudeus-memory` project directory.
    *   **Example (macOS):**
        ```json
        "command": "node",
        "args": ["/Users/yourusername/Projects/claudeus-memory/dist/index.js"]
        ```
    *   **Example (Windows):**
        ```json
        "command": "node",
        "args": ["C:\\Projects\\claudeus-memory\\dist\\index.js"]
        ```
...
4. Click the <img src="https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/claude-desktop-mcp-plug-icon.svg"/> icon. You should see "claudeus-memory" in the list of connected servers.
...
*   **Server not appearing in Claude:**
    ...
    *   Check the Claude Desktop logs for errors:
        *   **macOS:** `~/Library/Logs/Claude/mcp-server-claudeus-memory.log` (and `mcp.log`)
...
To install Claudeus Memory for Claude Desktop automatically via [Smithery](https://smithery.ai/protocol/claudeus-memory):

```bash
npx @smithery/cli install claudeus-memory --client claude
```

## Advanced Configuration
Claudeus Memory offers several ways to customize its behavior beyond the basic setup:
